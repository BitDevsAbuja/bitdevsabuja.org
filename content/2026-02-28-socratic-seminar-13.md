+++
title = "Socratic Seminar #13"
template = "post.html"
[extra]
+++

### Location

The event is hosted at **Respetech** by nully0x:

Block B, AUJ Complex, CBD Abuja

[Map](https://maps.app.goo.gl/eP2YgGgxkhDTn5Ts8)  
---

### Agenda


### Announcements

We are excited to introduce our thirteenth [Socratic Seminar](/about)! A special thank you to our sponsor [BTrust](https://www.btrust.tech/) for food, refreshments, and [Bitnob](https://bitnob.com) for hosting the meetup. 

### Reminders

- We prefer no photos and no videos during the event
- [Chatham House Rule](https://www.chathamhouse.org/about-us/chatham-house-rule)
- Leave the meeting space as clean as you found it
- Suggest topics for the next Socratic Seminar! by creating an issue on our repo [BitDevs Repo](https://github.com/BitDevsAbuja/bitdevsabuja.org/issues) [Where to find topics?](/about/find-topics)

---

### In Bitcoin

---

MC: nully0x

- [BIP for Output Script Descriptor Annotations](https://bitcoinops.org/en/newsletters/2026/02/27/)
  - Discussing the latest proposal to attach metadata to descriptors to improve scanning efficiency for silent payments.
- [The Great Script Restoration](https://bitcoinops.org/en/topics/op_cat/)
  - Updates on the progress of `OP_CAT` and its implications for covenants on Bitcoin.

---

### In Lightning

---

MC: Orion
- [The IMMORTAN LIBRARY and Hosted Lightning Channels](https://github.com/btcontract/immortan)
  - **Presenter: Orion**
  - IMMORTAN is a mobile-focused Lightning protocol stack for designing lightweight nodes.
  - Discussion on Hosted Channels as a custodial solution for anonymous and cost-effective onboarding in local Bitcoin economies

---

### Refreshment Break, Chill and Chat

---

### Book Club Review

MC: Eni

- Book and Research club conversation updates

---

### Where to learn

- [But how does bitcoin actually work? (3Blue1Brown)](https://www.youtube.com/watch?v=bBC-nXj3Ng4)
  - Visual explanation of cryptographic primitives, hashing, and proof-of-work
- [Bitcoin Whitepaper](https://bitcoin.org/bitcoin.pdf)
  - The 9-page paper that started it all — worth revisiting
- [Learn Me a Bitcoin](https://learnmeabitcoin.com/)
  - Interactive guide covering transactions, blocks, and the UTXO model
- [Mastering Bitcoin (Open Edition)](https://github.com/bitcoinbook/bitcoinbook)
  - Chapters 1-4 for foundational concepts


---

### News

MC: nully0x

  - [Btrust Builders Pathways](https://pathways.btrust.tech/)
  - [Bitcoin Optech Newsletter #394](https://bitcoinops.org/en/newsletters/2026/02/27/)
  - Remember to contribute [Good First Issue](https://bitcoindevs.xyz/good-first-issues)
  - Make an headway with [Btrust Grant](https://www.btrust.tech)
  - Catch up with weekly happenings in Bitcoin ecosystem via [tldr](https://tldr.bitcoinsearch.xyz/)
  - LARP Program

---

### Events

MC: nully0x

- [bitcoin++ open source edition](https://btcplusplus.dev/conf/nairobi)
- [Bitcoin Dakar 2026](https://bitcoindakar.com/)